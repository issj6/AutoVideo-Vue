	...
	<div v-if="is_uploaded" class="func-args-box">
		...
		<Form :model="args" :label-width="100">
			...
			<Space>
				...
				<FormItem label="目标语言:">
					<Select class="select-w" v-model="args.target_lang">
						<Option v-for="lang in langs" :value="lang.key">{{lang.value}}</Option>
					</Select>
				</FormItem>
			</Space>
			<!-- 选择发音人 -->
			<FormItem label="发音人:">
				<Select style="width: 300px;" v-model="args.speaker">
					<Option  v-for="speaker in speakers" :value="speaker.key">{{speaker.value}}</Option>
				</Select>
			</FormItem>
			<!-- 语速 -->
			<FormItem label="语速:">
				<Slider style="width: 400px;" v-model="args.voice_speed" 
				:min="0.5"
				:max="2.0"
				:step="0.01"
				show-input></Slider>
			...
			<!-- 语句间隔 -->
			<FormItem label="语句间隔:">
				<Slider style="width: 400px;" v-model="args.statement_interval_rate" 
				:min="0.75"
				:max="1.25"
				:step="0.01"
				show-input></Slider>
			...
			<!-- 是否核对字幕 -->
			<!-- <FormItem label="核对字幕:">
				<RadioGroup v-model="args.check_subtitle">
					<Radio label="y">人工核对</Radio>
					<Radio label="n">直接使用</Radio>
				</RadioGroup>
			</FormItem> -->
			<!-- 原音频 -->
			<FormItem label="原音频:">
				<RadioGroup v-model="args.o_audio">
					<Radio label="silent">静音</Radio>
					<Radio label="bgaudio">保留背景音</Radio>
				</RadioGroup>
			</FormItem>
			<Space>
				<!-- 添加字幕 -->
				<FormItem label="新字幕:">
					<RadioGroup v-model="args.new_subtitle">
						<Radio label="no">不添加</Radio>
						<Radio label="center-bottom">底部居中</Radio>
						<Radio label="center">整体居中</Radio>
						<Radio label="center-top">顶部居中</Radio>
					</RadioGroup>
				</FormItem>
				<FormItem label="字号:" :label-width="60">
					<Select style="width: 80px;" v-model="args.font_size">
						<Option  v-for="fontSize in [16,20,24,28,32,36,40,44,46,52,56]" :value="fontSize">{{fontSize}}</Option>
					</Select>
				</FormItem>
			</Space>
			<!-- 每句字符 -->
	...
	export default {
		data(){
			return{
				...
				speakers:[
					{'key':'en-US-AndrewNeural',value:'Andrew-Neural-英语'}
				]	
			}
		},
		methods:{
